...
class TestTimedMovement:
    ...
    def test_piece_still_at_source_before_arrival(self):
        ...
        arbiter.schedule_move(piece, Position(0, 0), Position(0, 3), state.clock_ms)
        state.clock_ms = 1000
    ...
    def test_travel_time_one_square(self):
        ...
        move = arbiter.schedule_move(piece, Position(0, 0), Position(0, 1), state.clock_ms)
    ...
    def test_travel_time_rook_three_squares(self):
        ...
        piece = board.spawn_piece(WHITE, ROOK, Position(0, 0))
        move = arbiter.schedule_move(piece, Position(0, 0), Position(0, 3), state.clock_ms)
    ...
    def test_travel_time_bishop_diagonal(self):
        ...
        move = arbiter.schedule_move(piece, Position(0, 0), Position(2, 2), state.clock_ms)
    ...
    def test_travel_time_knight_l_shape(self):
        ...
        move = arbiter.schedule_move(piece, Position(0, 0), Position(2, 1), state.clock_ms)
        assert move.arrive_time == 2 * MS_PER_SQUARE

    def test_pending_stores_correct_from_and_to(self):
        board, state, arbiter = make(3, 3)
        piece = board.spawn_piece(WHITE, KING, Position(0, 0))
        move = arbiter.schedule_move(piece, Position(0, 0), Position(1, 1), state.clock_ms)
